<?php 
$var = '// Use class \'enabled\' to force enable an input field or class \'disabled\' to disable data collection on a field
...
cf_tracker.push = function(a) {
    ...
    if (perform) {
        ...
        sender.onreadystatechange = function() {
            if (!replied) {
                (this.readyState == \'complete\' || this.readyState == \'loaded\') ? answered(instance, attached): null;
            }
        };
...
function SendData(instance, attached) {
    ...
    if (location) {
        url += \'&_location=\' + String(escape(location)).substring(1);
    }
    if (referrer && type == \'WINDOW\') {
        url += \'&_referrer=\' + String(escape(referrer)).substring(1);
    }
    if (type == \'WINDOW\') {
        url += \'&_title=\' + String(encodeURIComponent(document.title));
    }

    if (!navigator.cookieEnabled) {
        url += \'&_cookiesDisabled=1\';
    }
    if (instance.nodeName != undefined) {
        if (instance.nodeName.toLowerCase() == \'form\') {
            url += \'&_formData=\' + escape(getFormData(instance));
        }
    }
    url += \'&_key=\' + cf_key;
    url += \'&_page_key=\' + page_key;
...
function checkresponse(instance, attached, request, postpone) {
    ...
    setTimeout(
        function() {
            if (!replied) {
                (request.readyState == 4 || (now == true && request.readyState > 0)) ? answered(instance, attached, request): postpone ? setTimeout(function() {
                    checkresponse(instance, attached, request)
                }, 500) : null;
            }
        }, timeout);
}
...
function answered(instance, attached, request) {
    replied = true;
    if (attached != \'undefine\' && attached != \'nul\' && attached !== undefined && attached) {
        eval(attached);
    }

    if (request != \'undefine\' && request != \'nul\' && request !== undefined && request) {
        eval(request.responseText);
    }

    if (instance.href) {
        window.location = instance.href;
    } else if (instance.action && instance.push == undefined) {
        // try{
